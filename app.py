...
st.set_page_config(page_title="ALPHA TERMINAL FIX", layout="wide")

st.markdown("""
    <style>
    .stApp { background-color: #000000; }
    
    /* Menu tendina: Nero su Bianco */
    div[data-baseweb="select"] > div { background-color: #FFFFFF !important; color: #000000 !important; }
    div[data-baseweb="popover"] li { color: #000000 !important; background-color: #FFFFFF !important; }

    /* Testi Giallo Oro con ombra */
    ...
    /* Linea di separazione tra i match */
    hr { border: 1px solid #FFD700 !important; }
    </style>
    ...
    if st.button("ESEGUI SCANSIONE"):
        ...
        if status == 200:
            ...
            for match in data:
                ...
                m2 = next((b['markets'] for b in bookies if b['key'].lower() in ['betfair', 'betfair_ex']), bookies[1]['markets'])
                ...
                if is_good_over or is_good_lay:
                    found += 1
                    # --- DISEGNO DEL MATCH ---
                    st.markdown(f"### 🏟️ {match['home_team'].upper()} vs {match['away_team'].upper()}")
                    ...
                    with col1:
                        ...
                        if is_good_lay:
                            ...
                            st.write("❌ Requisito non soddisfatto")
                    ...
                    with col3:
                        ...
                        st.write(f"Quota Betfair: {q_o_b if is_good_over else q_d_b}")
                    ...
                    st.markdown("---") # Linea gialla tra un match e l'altro

            if found == 0:
                st.warning("Nessun match trovato con i criteri attuali.")
